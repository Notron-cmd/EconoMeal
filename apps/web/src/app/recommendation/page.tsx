...
import { Heart, ChefHat, RefreshCw, Sparkles, Zap, Timer, DollarSign, Soup, Lightbulb, ArrowLeft } from "lucide-react"
...
export default function RecommendationPage() {
  ...
  const [activeTab] = useState("ingredients")

  return (
    <div className="min-h-screen bg-background pb-32">
      <div className="px-5 pt-4 pb-4 space-y-5">
        {/* Hero Card */}
        <section className="bg-card rounded-[1rem] overflow-hidden shadow-[0px_10px_30px_rgba(28,25,23,0.04)]">
          <div className="relative h-72 w-full overflow-hidden">
            <div
              className="w-full h-full bg-cover bg-center transform hover:scale-105 transition-transform duration-700"
              style={{
                backgroundImage: "url('https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=600&h=400&fit=crop')",
              }}
            />
          ...
          <div className="p-5 space-y-4">
            <div className="flex justify-between items-start">
              ...
              <Heart className="w-6 h-6 text-on-surface-variant cursor-pointer hover:text-primary transition-colors" />
            </div>

            <div className="flex flex-wrap gap-2">
              <Badge icon={DollarSign} text="$2.50/serving" />
              <Badge icon={Zap} text="450 Cal" />
              <Badge icon={Zap} text="18g Protein" />
              <Badge icon={Timer} text="20 min" />
            </div>

            <div className="grid grid-cols-2 gap-3">
              <div className="flex items-center gap-3 p-3 bg-surface-container rounded-lg">
                <Soup className="w-5 h-5 text-on-surface-variant" />
                <div>
              ...
              <div className="flex items-center gap-3 p-3 bg-primary-container/20 border border-primary-container/30 rounded-lg">
                ...
                <div>
                  ...
                  <p className="text-[15px] text-on-surface-variant">Saves you <span className="font-bold text-primary">$4 today</span></p>
                </div>
              </div>
            </div>
        ...
        <section>
          ...
          <div className="flex gap-4 overflow-x-auto pb-2 -mx-5 px-5 hide-scrollbar">
            {alternatives.map((alt) => (
              <div
                key={alt.name}
                className="min-w-[260px] bg-card rounded-[1rem] overflow-hidden shadow-sm border border-surface-container shrink-0"
              >
                <div className="h-32 w-full bg-cover bg-center" style={{ backgroundImage: `url(${alt.image})` }} />
        ...
        <section className="bg-surface-container-low p-5 rounded-lg space-y-3">
          ...
          <div className="w-full h-4 bg-surface-container rounded-full overflow-hidden">
            <div className="h-full bg-gradient-to-r from-primary-light to-primary rounded-full" style={{ width: "65%" }} />
          </div>
